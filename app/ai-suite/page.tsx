...
// --- Tipe baru untuk menyimpan data gambar yang lengkap ---
...
// --- Komponen UI (ParameterInput, Accordion tidak berubah) ---
...
const ImageAnalyzer = ({ onPromptFromAnalysis }: { onPromptFromAnalysis: (prompt: string) => void }) => {
    ...
    const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) { setImageFile(file); setAnalysisResult(null); const reader = new FileReader(); reader.onloadend = () => setImagePreview(reader.result as string); reader.readAsDataURL(file); }
    };
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose}>
            <div className="bg-slate-800 border border-slate-700 rounded-lg max-w-4xl w-full max-h-[90vh] flex flex-col lg:flex-row gap-6 p-6 relative" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="flex-shrink-0 lg:w-1/2">
                    <div className="relative aspect-square w-full bg-slate-900 rounded-md overflow-hidden">
                        <Image src={imageData.url} alt={imageData.prompt} layout="fill" className="object-contain" />
                    </div>
                </div>
                ...
                <div className="flex-grow lg:w-1/2 overflow-y-auto pr-2 space-y-4">
                    ...
                    <div>
                        <label className="text-sm font-semibold text-slate-400">Prompt</label>
                        <p className="mt-1 text-base text-slate-200 bg-slate-700/50 p-3 rounded-md">{imageData.prompt}</p>
                    </div>
                    <div className="grid grid-cols-2 gap-4 text-sm">
                        <div>
                            <label className="font-semibold text-slate-400">Model</label>
                            <p className="text-slate-200">{imageData.model}</p>
                        </div>
                         <div>
                            <label className="font-semibold text-slate-400">Gaya Seni</label>
                            <p className="text-slate-200">{imageData.artStyle}</p>
                        </div>
                        <div>
                            <label className="font-semibold text-slate-400">Kualitas</label>
                            <p className="text-slate-200">{imageData.quality}</p>
                        </div>
                        <div>
                            <label className="font-semibold text-slate-400">Resolusi</label>
                            <p className="text-slate-200">{imageData.width} x {imageData.height}</p>
                        </div>
                         <div>
                            <label className="font-semibold text-slate-400">Seed</label>
                            <p className="text-slate-200 break-all">{imageData.seed}</p>
                        </div>
                    </div>
                    <div className="pt-4 flex flex-col sm:flex-row gap-2">
                        ...
                        <button 
                            onClick={() => { navigator.clipboard.writeText(imageData.prompt); toast.success('Prompt disalin!'); }}
                            className="w-full sm:w-auto flex-1 bg-slate-600 text-white font-bold py-2 px-4 rounded-lg flex items-center justify-center gap-2 hover:bg-slate-700 transition">
...
function AISuitePageContent() {
    ...
    // State untuk menyimpan data gambar yang lengkap
    ...
    useEffect(() => {
        ...
        if (urlPrompt && urlPrompt !== prompt) { setPrompt(urlPrompt); toast.success('Prompt dari halaman utama dimuat!'); }
    }, [searchParams, prompt]);
                ...
                <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
                    ...
                    <div className="lg:col-span-3">
                         {generatedImages.length > 0 && (
                            <div>
                                <h2 className="text-3xl font-bold mb-4 text-white">Hasil Generasi</h2>
