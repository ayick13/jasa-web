...
// --- Tipe Data & Konstanta ---
const imageGenModels = ['flux', 'turbo', 'dall-e-3'] as const;
...
const ImageDetailModal = ({ isOpen, onClose, imageData }: { isOpen: boolean, onClose: () => void, imageData: GeneratedImageData | null }) => { if (!isOpen || !imageData) return null; return ( <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose}> <div className="bg-slate-800 border border-slate-700 rounded-lg max-w-4xl w-full max-h-[90vh] flex flex-col lg:flex-row gap-6 p-6 relative" onClick={(e) => e.stopPropagation()}> <button onClick={onClose} className="absolute top-4 right-4 text-slate-400 hover:text-white transition-colors z-10"><X size={24} /></button> <div className="flex-shrink-0 lg:w-1/2"><div className="relative aspect-square w-full bg-slate-900 rounded-md overflow-hidden"><Image src={imageData.url} alt={imageData.prompt} layout="fill" className="object-contain" /></div></div> <div className="flex-grow lg:w-1/2 overflow-y-auto pr-2 space-y-4"> <h3 className="text-2xl font-bold text-white">Detail Generasi</h3> <div><label className="text-sm font-semibold text-slate-400">Prompt</label><p className="mt-1 text-base text-slate-200 bg-slate-700/50 p-3 rounded-md">{imageData.prompt}</p></div> <div className="grid grid-cols-2 gap-4 text-sm"> <div><label className="font-semibold text-slate-400">Model</label><p className="text-slate-200">{imageData.model}</p></div> <div><label className="font-semibold text-slate-400">Gaya Seni</label><p className="text-slate-200">{imageData.isDalle ? 'N/A' : imageData.artStyle}</p></div> <div><label className="font-semibold text-slate-400">Kualitas</label><p className="text-slate-200">{imageData.isDalle ? 'N/A' : imageData.quality}</p></div> <div><label className="font-semibold text-slate-400">Resolusi</label><p className="text-slate-200">{imageData.width} x {imageData.height}</p></div> <div><label className="font-semibold text-slate-400">Seed</label><p className="text-slate-200 break-all">{imageData.seed}</p></div> </div> <div className="pt-4 flex flex-col sm:flex-row gap-2"> <a href={imageData.url} download={`ayick-ai-${imageData.seed}.png`} className="w-full sm:w-auto flex-1 bg-cyan-600 text-white font-bold py-2 px-4 rounded-lg flex items-center justify-center gap-2 hover:bg-cyan-700 transition"><Download size={16}/> Unduh</a> <button onClick={() => { navigator.clipboard.writeText(imageData.prompt); toast.success('Prompt disalin!'); }} className="w-full sm:w-auto flex-1 bg-slate-600 text-white font-bold py-2 px-4 rounded-lg flex items-center justify-center gap-2 hover:bg-slate-700 transition"><Copy size={16}/> Salin Prompt</button> </div> </div> </div> </div> ); };
const DalleApiKeyModal = ({ isOpen, onClose, onSave }: { isOpen: boolean, onClose: () => void, onSave: (key: string) => void }) => { const [apiKey, setApiKey] = useState(''); if (!isOpen) return null; const handleSave = () => { if (apiKey.trim()) { onSave(apiKey.trim()); } else { toast.error("API Key tidak boleh kosong."); } }; return ( <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose}> <div className="bg-slate-800 border border-slate-700 rounded-lg max-w-md w-full p-6 relative space-y-4" onClick={(e) => e.stopPropagation()}> <button onClick={onClose} className="absolute top-4 right-4 text-slate-400 hover:text-white transition-colors z-10"><X size={20} /></button> <div className="flex items-center gap-3"> <KeyRound className="text-yellow-400" size={24}/> <h3 className="text-xl font-bold text-white">Masukkan API Key OpenAI</h3> </div> <p className="text-sm text-slate-400">Model DALL-E 3 memerlukan API Key OpenAI Anda sendiri untuk berfungsi. Key Anda hanya akan disimpan sementara di browser Anda untuk sesi ini.</p> <input type="password" value={apiKey} onChange={(e) => setApiKey(e.target.value)} placeholder="sk-..." className="w-full bg-slate-700 border-slate-600 rounded-md p-2 text-white focus:outline-none focus:ring-2 focus:ring-cyan-500" /> <a href="https://platform.openai.com/api-keys" target="_blank" rel="noopener noreferrer" className="text-xs text-cyan-400 hover:underline flex items-center gap-1"> Bagaimana cara mendapatkan API Key? <ExternalLink size={14}/> </a> <div className="flex justify-end gap-2 pt-2"> <button onClick={onClose} className="bg-slate-600 hover:bg-slate-700 text-white font-bold py-2 px-4 rounded-lg text-sm">Batal</button> <button onClick={handleSave} className="bg-cyan-600 hover:bg-cyan-700 text-white font-bold py-2 px-4 rounded-lg text-sm">Simpan & Lanjutkan</button> </div> </div> </div> ); };
...
function AISuitePageContent() {
    ...
    const saveHistory = (newPrompt: string) => { const newItem: HistoryItem = { id: `hist-${Date.now()}`, prompt: newPrompt, timestamp: new Date().toLocaleString('id-ID') }; const updatedHistory = [newItem, ...history].slice(0, 50); setHistory(updatedHistory); try { localStorage.setItem('generationHistory', JSON.stringify(updatedHistory)); } catch (e) { console.error("Failed to save history:", e); } };
            ...
            if (imageGenModel === 'dall-e-3') {
                ...
                const promises = Array.from({ length: batchSize }, () => {
                    ...
                    return fetch(imageUrl)
                        ...
                        .then(blobUrl => {
                            // Perbaikan Tipe TypeScript di sini
                            if (!blobUrl) return null;
                            const imageData: GeneratedImageData = { url: blobUrl, prompt: finalPrompt, model: imageGenModel, artStyle, quality, width: imageWidth, height: imageHeight, seed };
                            return imageData;
                        })
                        .catch(() => null);
                });
