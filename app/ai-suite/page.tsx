    ...
    useEffect(() => {
        const fetchModels = async () => {
            try {
                ...
                const data = await response.json(); setAvailableModels(data);
        ...
        <div className="flex flex-col h-[500px] bg-slate-900/50 rounded-lg">
                ...
                {messages.map(msg => (
                    <div key={msg.id} className={`flex gap-2 text-sm ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                        ...
                        <div className={`p-2.5 rounded-lg max-w-[85%] ${msg.role === 'user' ? 'bg-sky-600 text-white rounded-br-none' : 'bg-slate-700 text-slate-300 rounded-bl-none'}`}>
                            ...
                            {msg.role === 'assistant' && msg.content && msg.content !== "..." && !msg.content.includes("kesalahan") && <button onClick={() => onPromptFromChat(msg.content)} className="mt-2 text-xs font-semibold text-cyan-400 hover:underline">Gunakan sebagai Prompt</button>}
                        </div>
                    </div>
                ))}
            </div>
            <div className="p-4 border-t border-slate-700">
                <div className="flex items-center gap-2">
                    <button onClick={() => imageInputRef.current?.click()} title="Upload Gambar" className="p-2 rounded-full text-slate-400 hover:bg-slate-700 hover:text-white transition"><Paperclip size={20}/></button>
                    <input type="text" value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSendMessage()} placeholder="Tanya sesuatu atau upload gambar..." className="w-full bg-slate-700 text-white rounded-full py-2 px-4 focus:outline-none focus:ring-2 focus:ring-cyan-500" />
...
const TextToAudioConverter = () => {
    ...
    const handleGenerateAudio = async () => {
        ...
        } catch (error: any) { toast.error(error.message, { id: toastId }); } finally { setIsLoading(false); }
    };
                ...
                <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
                    ...
                    <div className="lg:col-span-3">
                        <div className="bg-slate-800/20 backdrop-blur-sm p-4 rounded-2xl shadow-inner shadow-black/20 border border-slate-700 h-full min-h-[400px] lg:min-h-0 flex flex-col">
                            {isLoading ? ( <div className="m-auto text-center text-slate-400"><Sparkles className="mx-auto h-12 w-12 animate-pulse" /><p className="mt-4 font-semibold">Menciptakan keajaiban...</p></div> ) : generatedImages.length > 0 ? (
                            ...
                            ) : ( <div className="m-auto text-center text-slate-500"><ImageIcon size={64} className="mx-auto" /><p className="mt-4 font-semibold">Hasil gambar Anda akan muncul di sini</p><p className="text-sm">Tulis prompt dan klik "Generate Image"</p></div> )}
                        </div>
                    </div>
                </div>
            </main>
