// File: app/register/page.tsx (Ditambahkan SocialLoginButtons & Show/Hide Password)
...
import { Eye, EyeOff } from 'lucide-react'; // <-- Pastikan ini sudah diimpor

function RegisterForm() {
    const router = useRouter();
    const [data, setData] = useState({ name: '', email: '', password: '' });
    const [isLoading, setIsLoading] = useState(false);
    const [showPassword, setShowPassword] = useState(false); // State baru untuk tampil/sembunyi password
        ...
        <div className="bg-slate-800 p-8 rounded-lg shadow-lg w-full max-w-md">
            ...
            <form onSubmit={registerUser} className="space-y-6">
                ...
                <div>
                  ...
                  <div className="relative"> {/* Div ini harus ada dan membungkus input & button */}
                    <input
                      id="password"
                      name="password"
                      type={showPassword ? 'text' : 'password'} /* Tipe berubah berdasarkan state showPassword */
                      required
                      value={data.password}
                      onChange={(e) => setData({ ...data, password: e.target.value })} /* <<-- Handler ini HARUS ADA -->> */
                      className="mt-1 block w-full bg-slate-700 border border-slate-600 rounded-md shadow-sm py-2 px-3 text-white focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 pr-10" /* pr-10 untuk memberi ruang tombol */
                    />
