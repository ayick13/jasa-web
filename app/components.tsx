              ...
              if (link.type === 'submenu') {
                return (
                  <div key={link.id} className="w-full">
                    <span className="block px-3 py-2 font-semibold text-slate-700 dark:text-slate-200 items-center gap-2">
                      <link.icon className="w-5 h-5" />
                      {link.label}
                    </span>
                    {link.submenu.map((sub) => (
...
export const HeroSection: React.FC<{ onNavClick: (section: HomeSection) => void; sectionRef: React.RefObject<HTMLElement> }> = ({ onNavClick, sectionRef }) => {
    ...
    return (
        <section ref={sectionRef} id="home" className="min-h-screen flex items-start md:items-center justify-center bg-grid-pattern animate-fade-in pt-20 md:pt-0">
            <Toaster />
