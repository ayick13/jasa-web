...
const skillsData = [
    ...
    { name: "Firebase" }, { name: "Git & GitHub" }, { name: "FontAwesome" },
    { name: "Bootstrap" }, { name: "WordPress" }, { name: "Elementor" },
    { name: "WordPress Plugins" }, { name: "Digital Marketing" }, { name: "Photoshop Design" }
];
          ...
          <div className="flex items-center gap-4">
            <nav className="hidden md:flex items-center space-x-1">
              {navLinks.map(link => {
                if (link.type === 'submenu') {
                  ...
                  return (
                    <div key={link.id} className="relative group pb-4 -mb-4">
                      ...
                      <div className="absolute left-0 pt-4 w-56 bg-transparent pointer-events-none group-hover:pointer-events-auto">
                        <div className="bg-white dark:bg-slate-800 rounded-md shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-opacity">
                ...
                const isActive = (link.type === 'scroll' && link.id === currentSection && pathname === '/') || (link.type === 'page' && pathname.startsWith(link.href));
              ...
              if (link.type === 'submenu') {
                return (
                  <div key={link.id} className="w-full">
                    <span className="flex items-center gap-3 px-3 py-2 font-semibold text-slate-700 dark:text-slate-200">
                      <link.icon className="w-6 h-6" />
                      {link.label}
                    </span>
                    {link.submenu.map((sub) => (
                      <Link key={sub.id} href={sub.href} onClick={() => setIsOpen(false)} className="w-full block pl-12 pr-6 py-2 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 text-base">
                        {sub.label}
                      </Link>
                    ))}
                  </div>
...
export const HeroSection: React.FC<{ onNavClick: (section: HomeSection) => void; sectionRef: React.RefObject<HTMLElement> }> = ({ onNavClick, sectionRef }) => {
    ...
    return (
        <section ref={sectionRef} id="home" className="min-h-screen flex items-center justify-center bg-grid-pattern animate-fade-in pt-20 md:pt-0">
            <Toaster />
...
export const BlogSection: React.FC<{ sectionRef: React.RefObject<HTMLElement>; recentPosts: Post[] }> = ({ sectionRef, recentPosts }) => {
    const postsToRender = Array.isArray(recentPosts) ? recentPosts : [];
